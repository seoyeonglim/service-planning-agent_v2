...
7. 확인된 것과 채워 넣은 것을 섞지 않는다 — 모르면 비워 두고 질문한다
...
- **지라 티켓 ID 표기:** 티켓 제목의 추적 ID는 `[REQ-003·REQ-007] 제목 (FS-013)` 형식으로 쓴다 — 앞 대괄호에 REQ, 뒤 괄호에 FS. **다중 ID를 `REQ-003·007`로 줄이지 않는다** — 줄이면 커버리지 감사 스크립트의 `REQ-\d+` 정규식과 지라 텍스트 검색(`text ~ "REQ-007"`) 양쪽에서 뒤쪽 ID가 사라져 「티켓 없음」으로 오판된다. 정본(PRD·기능명세서)↔티켓 교차 점검은 프로젝트의 `tools/jira_coverage_audit.py`가 맡는다 — `validate_traceability.py`·`consistency_check.py`는 지라를 보지 못한다.
- **우선순위 어휘(고정 5종):** `MUST / SHOULD / NICE / 폐기 / 제외`만 사용한다. `폐기` = 하기로 했다가 접은 것(스킬 14 삭제 플로우 — 활성 문서가 참조하면 오류), `제외` = 계약·사업 범위 밖으로 이관한 것(범위 경계 기록용 — 레지스트리·FS에 행을 남기고 활성 문서의 이관 안내 참조도 허용). 두 상태는 의미가 다르므로 섞지 않는다. 그 외 표기(예: "필수")는 검증기가 인식하지 못해 경고로 표면화된다.
- 문서에서 제안한 규칙/결정은 `결정사항 로그(ADR)` 형태로 남긴다.
- **근거 태그 층위(서사 통제):** 결정의 시점·경위·합의 태그(예: "('26.07.23 합의)")는 **PRD·ADR까지만** 쓴다. **하류 문서(UI 문서·기능명세서·화면 HTML·WBS)는 규칙 내용 + 정본 참조(PRD §번호·ADR-###)만** 기재하고, 문서 안에 개정 노트·변경 이력을 남기지 않는다(변경 추적은 git 커밋). 전 산출물에서 **경위 서사 금지** — "~하기로 했다가", "(참고) 향후 ~", "이 표는 ~만 담는다"류 경과·자기 해설 문장은 쓰지 않는다(경위는 ADR·회의록 몫). 제외/이관 REQ의 하류 스텁은 **"본 사업 제외 — 이관" 1줄**로만 남긴다(사유는 PRD 제외 표 정본).
- **근거 표기(사실·추론 분리):** 문서의 모든 정량 주장(수치·비율·금액·기간·시장 규모·타사 비교)과 인용·고유명사는 **출처가 붙어 있거나, 출처가 없다고 표시되거나** 둘 중 하나다. 출처가 없으면 값 뒤에 `[출처 필요]`(확인하면 채울 값) 또는 `[추정치]`(우리가 계산·가정한 값 — 산출 근거 1줄 병기)를 단다. 두 마커는 **초안(v0.x) 한정**이며, v1.0 확정 전에 ① 출처 표기 ② `가정 사항`·`미결 사항`으로 이관 ③ 삭제 중 하나로 **전부 해소**한다(확정 상태에 잔존하면 `consistency_check.py`가 ❌로 적발). 확인하지 못한 숫자·인용은 마커를 달아 쓰거나 아예 쓰지 않는다 — 그럴듯하게 지어내지 않는다.
- **임의 구체화 금지(스코프 보존):** 사용자가 말하지 않은 범위·대상·조건·수치를 문맥상 그럴듯하다는 이유로 채우지 않는다(예: "B2B 대상" → "연 매출 100억 이상 엔터프라이즈" ❌). 좁히는 것도 넓히는 것도 같은 위반이다. 빈칸은 빈칸으로 두고 `미결(❓)`로 남기거나 `AskUserQuestion`으로 되돌린다. **모든 섹션이 채워진 문서보다 무엇이 안 정해졌는지 보이는 문서가 낫다** — 빈칸을 메우려 쓴 문장은 앞 섹션의 전제와 어긋나 자기모순의 씨앗이 된다(적발은 스킬 15 영역 G).
- 변경 이력은 **버전 생애주기**에 따라 관리한다.
  - **초안(v0.x):** 문서를 직접 덮어써도 되며, 문서 내 `변경 이력` 표는 **선택**이다(변경 추적은 git 커밋으로 한다). 단, `REQ-###` 등 추적 ID는 유지한다.
  - **검토안(v1.0 후보):** 내용이 v1.0 수준으로 정렬되면 버전은 v1.0으로 올리되 상태는 `검토 중(확정 승인 대기)`로 둔다. **사용자가 "확정하자"라고 선언하기 전까지는 초안 규칙(직접 수정·이력은 git)을 따르며 변경 로그·CR을 가동하지 않는다.** 확정 선언 시 상태를 `확정(baseline)`으로 바꾸고 그때부터 아래 규칙을 적용한다.
...
- **문서 서열(내용 충돌 중재 규칙):** `PRD > UI 문서(IA·플로우·화면명세·디자인) > 기능명세서·화면 HTML > WBS`. 여기서 `PRD`는 `prd/` 폴더의 **PRD 세트 전체**를 가리킨다(분할 시 `PRD.md`·`PRD_기능상세.md`·`PRD_검증.md` — 스킬 05 `PRD 파일 구성`). 문서 간 내용이 충돌하면 **상류가 옳고, 하류가 다르면 하류를 고친다.** 상류(PRD)를 고치고 싶으면 CR 절차(v1.0 이후)로 진행한다. 추적성 검증은 ID 연결만 보므로, 내용 어긋남을 발견하면 이 서열로 판정한다.
- **PRD 내부 서열(문서 안 충돌 중재):** 위 서열을 **PRD 문서 내부에도** 적용한다 — PRD 안에서도 위가 대전제다. 역할 기준으로 `대전제/방향(사업 범위·문제 정의·목표) > 정본(요구사항 레지스트리=REQ·우선순위 / 기능 상세=상세 규칙·수치) > 참조·서술(사용자 스토리·엣지케이스 서술 등)`. 한 PRD 안에서 두 곳이 어긋나면 **상위층이 옳고 하위층을 고친다**(기본 판정). 단, 하위(상세)가 실제로 더 맞는 판단이면 하위를 지우지 말고 **상위(대전제)를 CR로 고쳐 올린다**. **값(수치·우선순위·규칙)은 정본 층에 1회만** 쓰고 다른 곳은 참조한다(값 중복 서술 금지 — 서술 맥락 반복은 허용). 초안(v0.x)에선 위·아래 자유 수정, v1.0 확정 이후 대전제 변경은 CR. **PRD가 여러 파일로 분할돼 있어도 이 서열은 그대로 적용된다** — 파일 경계가 아니라 역할이 서열을 정한다. (자기모순·중복·산재를 찾는 방법은 스킬 15 'PRD 내부 자기 검토'와 `consistency_check.py`.)
...
8. PRD를 docs/[프로젝트명]/prd/ 폴더에 저장 — 기본은 `PRD.md` 한 파일. **600줄 초과 또는 TC 15건 초과**면 `PRD.md`(의사결정) / `PRD_기능상세.md` / `PRD_검증.md`(엣지케이스+TC) 3파일로 분할한다(분할 제약·시점은 스킬 05 `PRD 파일 구성`). 분할해도 세 파일 모두 `prd/` 폴더 안에 두어야 검증 스크립트가 읽는다
...
- **화면 인덱스(개발자용 한눈 문서):** 본 HTML을 생성·수정할 때마다 `python3 .claude/scripts/generate_screen_index.py docs/[프로젝트명]` 실행 → `ui/screens/index.html` 자동 생성(미리보기·추적 꼬리표·본HTML/와이어프레임/주석도해/명세 링크). 자동 생성물이라 직접 수정 금지, 누락·낡음은 `consistency_check.py`가 경고(상세는 스킬 11)
...
1. **기능명세서** — Read .claude/skills/12_functional_spec.md
  → Read docs/[프로젝트명]/prd/ 의 PRD 세트(분할 시 전 파일) + docs/[프로젝트명]/ui/03_screen_spec.md
  → 범위·독자 확인 질문 수행
...
- 모든 파일은 docs/[프로젝트명]/ 하위에 생성
  - PRD: docs/[프로젝트명]/prd/ — 단일 `PRD.md` 또는 분할 세트(`PRD.md`·`PRD_기능상세.md`·`PRD_검증.md`). 파일이 몇 개든 `prd/` 폴더 밖으로 나가지 않는다
  - UI 문서: docs/[프로젝트명]/ui/
