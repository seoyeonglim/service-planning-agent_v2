...
> **총칙:** 모든 시각 산출물은 HTML/Mermaid로 직접 작성한다 — 외부 이미지 생성 API 호출 금지.
...
1. **각 화면의 본 HTML 생성 전 — 화면별로 실행 (항상, 단 요소 3개 이하 단순 화면은 생략 가능)**
  - 텍스트 명세만으로는 레이아웃 구조 검증이 불가능하므로
  - 각 화면의 구조를 Mermaid graph로 표현하여 사용자 확인 (Phase 3 "화면당 생성 순서" ①과 동일 단계)
...
**실행 조건:** 1단계 Mermaid 확인 완료 + 사용자 승인 후, 화면별 ③ 주석 HTML 도해 생성 직전마다 실행 (①→②→③→④ 순서)
...
2. 해당 화면 명세서 문서에 링크로 연결
  ...
  ```
3. 사용자에게 와이어프레임을 보여주고 "이 방향으로 진행할까요?" 확인
4. 사용자 승인 → ③ 주석 HTML 도해 생성 진행 (이후 ④ 본 HTML)
