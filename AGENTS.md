...
- 모든 파일은 `docs/[프로젝트명]/` 하위에 생성
  ...
  - 로컬 전용 폴더: `docs/[프로젝트명]/sow/`(계약 문서) · `diagrams/` · `_archive/`(과거 버전) — 산출물이지만 git 추적하지 않는다(형상관리 화이트리스트 참조)
...
### 내용 정합성 검증 (게이트 통과 직전·대규모 변경 후 권장)
...
- **기계 판정 항목**(위 스크립트): FS 우선순위 상속, FS↔REQ 단계 일치, MUST FS의 WBS 배치, 화면별 산출물(와이어프레임·본 HTML) 존재, 폐기 REQ의 활성 참조, ID 중복 선언
